...
7. **LCP preload for homepage** — `build.js` always injects a `<link rel="preload" as="image" fetchpriority="high">` for the first collection's cover image into `_site/index.html`, whether or not static mode is enabled. In non-static mode this lets the browser start fetching the LCP cover while JS is still loading the manifest.
