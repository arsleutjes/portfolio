...
[![Live Site](https://img.shields.io/badge/Live%20Site-arsleutjes.nl-blue)](https://arsleutjes.nl)
...
4. **About page** — `content/about.md` is rendered to HTML at build time and injected directly into `_site/about.html`. `content/profile.jpg` (if present) is copied to `_site/profile.jpg` and a `<link rel="preload" as="image" fetchpriority="high">` hint is added to `<head>` so the browser discovers the LCP profile photo immediately. The raw `.md` file is never included in `_site/`.
5. **Image optimisation** — every source image under `content/photos/` is processed with `sharp` into four WebP variants (400w, 800w, 1200w, 1920w at quality 85) and written to `_site/photos/`. For the cover image of each collection, an additional `{stem}-og.webp` (1200×630 centre-crop, quality 80) is generated for use in OG/Twitter meta tags — sized to stay under WhatsApp's 600 KB limit. Results are cached in `.image-cache/` (keyed by SHA-256 hash of the source file) so unchanged images are skipped on subsequent builds. Source-only files such as `meta.json` are not copied.
...
7. **LCP preload for homepage** — `build.js` injects `<link rel="preload" as="image" fetchpriority="high">` hints for the first three collection cover images into `_site/index.html` (both static and non-static modes). All three preloads carry `fetchpriority="high"` because any of the above-the-fold covers can be the LCP candidate on its target viewport. The second cover uses `media="(min-width: 481px)"` (visible on tablet+desktop) and the third uses `media="(min-width: 769px)"` (visible on desktop only), keeping mobile unaffected. In non-static mode this lets the browser start fetching above-the-fold covers while JS is still loading the manifest; in static mode these preloads are part of the full pre-render pass. Covers at indices 0–2 are rendered with `loading="eager" fetchpriority="high"` in the pre-rendered HTML and in JS-built grids.
8. **Static pre-render** (when `content/meta.json` sets `"static": true`) — the homepage cover grid is baked into `_site/index.html` and every collection gets its own `_site/collection/[slug]/index.html`. Each pre-rendered page includes a `<link rel="preload" as="image" fetchpriority="high">` for the LCP image and renders it with `loading="eager" fetchpriority="high"`, so the browser can fetch it without waiting for JavaScript.
