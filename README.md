...
7. **LCP preload for homepage** — `build.js` injects `<link rel="preload" as="image">` hints for the first three collection cover images into `_site/index.html` (both static and non-static modes). The first cover gets `fetchpriority="high"` (it is the LCP). The second cover is preloaded with `media="(min-width: 481px)"` (visible on tablet+desktop) and the third with `media="(min-width: 769px)"` (visible on desktop only), keeping mobile unaffected. In non-static mode this lets the browser start fetching above-the-fold covers while JS is still loading the manifest; in static mode these preloads are part of the full pre-render pass. Covers at index 1 and 2 are also rendered with `loading="eager"` (instead of lazy) in the pre-rendered HTML and in JS-built grids.
